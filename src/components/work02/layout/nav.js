...
const links = [
	...
	{ title: 'Products', href: '/work02/products', subtitle: '暮らしを彩る手作りの照明' },
	{ title: 'About', href: '/work02/about', subtitle: '照明のデザインから施工まで' },
]
			...
			>
				<ul className="flex flex-col gap-[1.5em] font-josefin text-4xl sm:text-5xl">
					{links.map(({ title, href, subtitle }) => (
						...
						>
							<NavLink title={title} subtitle={subtitle} href={href} closeNav={closeNav} />
						</motion.li>
					))}
				</ul>
			</motion.div>
...
// メニューのリンク部分
function NavLink({ title, subtitle, href, closeNav }) {
	return (
		<motion.div initial="offHover" whileHover="onHover">
			<Link href={href} onClick={closeNav} className="relative flex items-baseline gap-[0.75em]">
				{title}
